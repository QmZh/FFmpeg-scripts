@echo off
if "%~2"=="" (echo "inputs: input video file, start time(hh:mm:ss.xxx, must have ss, can omit others), frames(defaults to 5), use jpg?(y/*, jpg is smaller but not lossless), the equal length of the number part of the name that will be filled with leading 0's (there is a file name length limit)" & echo ----- & echo "Note that if there are picture files with the same names, they will be overwritten directly" & exit)
